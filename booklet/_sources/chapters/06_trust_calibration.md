# Chapter 6: Trust Calibration

Trust in an AI system is not a switch you flip. Think of it as a dial you calibrate.

When a GenAI engineer deploys an AI agent into an operational environment (an IT service desk, a network operations center, a clinical workflow) the central design challenge is not accuracy, latency, cost per token, or even safety in the abstract. The central challenge is ensuring that the humans who work alongside the agent trust it *exactly as much as it deserves to be trusted*. Not more. Not less. This chapter examines what trust in automated systems actually consists of, how it forms and breaks, and how to design interaction patterns that keep it properly calibrated. Note that this chapter addresses *operator-side* calibration: how humans interpret and act on AI confidence signals. Model-side calibration (whether the model's stated confidence matches actual accuracy) is a separate engineering problem; Chapter 8 provides a practical workflow for empirically calibrating model confidence against operational outcomes.
...
**Learned trust** is the layer that accumulates through direct experience with the specific system. It is the most powerful and the most designable. Merritt and Ilgen (2008) demonstrated that once people start working with a specific system, history-based trust quickly comes to dominate whatever disposition they arrived with. This finding has profound design implications: the onboarding experience is the period during which the operator's long-term trust calibration is being established, not merely an introduction.
...
> **Key insight:** Designing an AI agent to say "I'm not sure" is not a concession of weakness but a calibration mechanism. The goal is not to maximize the operator's confidence in every recommendation but to maximize the operator's accuracy in the decisions they make based on those recommendations.
...
This finding aligns with the learned trust layer in Hoff and Bashir's framework and points to a concrete design requirement: **track record dashboards**. These are not simple accuracy percentages but navigable histories that allow operators to build calibrated mental models of where the system succeeds and where it fails.
...
Trust in automated systems, once damaged, follows an asymmetric trajectory that every GenAI engineer must account for: it declines rapidly after a failure (often in a single event) but recovers slowly, over multiple successful interactions. The asymmetry between how trust is destroyed and how it is rebuilt is a long-standing finding in the risk literature, and De Visser, Pak, and Shaw (2018) built the trust-repair framework for human-machine interaction on top of it. A single high-visibility failure can erase weeks or months of earned trust.
...
Pak and Rovira (2023) modeled which repair actions should prove most durable (the empirical record on trust repair is genuinely mixed, which is what motivated their model): their prediction, grounded in persuasion theory, is that **substantive explanations produce more durable repair than emotional apologies**, because explanation engages the operator's reasoning about the system rather than their feelings about the event. An apology that carries explanatory content can work too; an apology without one decays. Engineers will find the prediction unsurprising, and it has direct implications for incident communication design.
...
4. **Graduated re-engagement.** After a significant failure, temporarily increase the level of human oversight. This is a calibration mechanism rather than a punishment: it lets the operator rebuild learned trust through direct observation.
...
A retrospective study using the MIMIC-III clinical dataset with an AI clinical decision support system illustrates the metric: recommendations at the 90--99% confidence level were overridden at a rate of only 1.7%. (One simulated study in one domain; take the number as an illustration of the measurement approach, not a benchmark.) The critical question then becomes whether the overrides that do occur at high confidence capture genuine system errors, which requires tracking override accuracy over time.
...
<a href="https://demos.barcik.training/demos/shortlist.html">The Shortlist</a> computes your own Weight-of-Advice score: review an AI-ranked hiring shortlist (two of the rankings are wrong for reasons only the full files reveal) and see how much of the final order was actually yours.
</div>
...
| Graduated autonomy during onboarding | Accounts for rapid shift from dispositional to learned trust | Merritt & Ilgen (2008): history-based trust dominates early | Start with human-in-the-loop for all actions; expand autonomy based on demonstrated calibration |
